...
def make_download_excel(project_name: str,
    ...
    """Export tables to Excel with comma-formatted strings."""
    output = BytesIO()
...
st.markdown(
f"""
- **문제 정의**: 프로젝트의 라인 단위 RFQ에서 금형/개발비 등의 고정비 상각과 제품 별 재료비 변동 때문에, “얼마에 제출해야 수주와 이익을 동시에 잡는지”가 매번 감으로 결정됨  
- **해결 방향**: 과거 RFQ 데이터의 *마진율 - 수주여부* 패턴을 학습하고, 신규 RFQ는 원가 엔진으로 @단위 원가를 계산한 뒤 마진을 스윕하여 의사결정  
...
# Sanity check table
st.subheader("Sanity Check: 단위원가 구성 Top5")
san = df.copy()
...
st.caption("Tip: 총원가가 비정상적으로 크면 material(투입량×단가), proc(가공비), subparts(plate_factor 등) 중 하나가 '단위'가 다른 경우.")
...
project_raw_profit = float(picked_rows["raw_profit"].sum())
...
col4.metric("수주 확정시 총 이익(합)", f"{project_raw_profit:,.0f} KRW")
